...
⚠️ CRITICAL CHECK - Examine CAREFULLY for logical contradictions
...
- Check for timing conflicts:
  ...
  * Example issue: "infected" (already) → "will be hospitalized" (future) ❌
  * Example correct: "in critical condition" → "will die" ✅
...
- Check for illogical relationships:
  ...
  * Example issue: "no treatment" → "will be hospitalized" ❌ (illogical)
  * Example correct: "no treatment" → "will die/deteriorate" ✅ (logical)
- Sub-result: [OK / ISSUE + explain]
...
- Check for severity mismatches:
  * Example issue: "executed" (death) → "hospitalized" (treatment) ❌
  * Example correct: "executed" → "die/killed" ✅
  * Example correct: "injured" → "wounded" ✅
- Sub-result: [OK / ISSUE + explain]
...
JSON:"""
            ...
            if result and isinstance(result, dict):
                ...
                if is_valid:
                    ...
                    if step3_issues:
                        print(f"   ⚠️ 逻辑问题: {', '.join(step3_issues)}")
