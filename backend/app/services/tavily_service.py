...
class TavilySearchService:
    ...
    def _initialize_client(self):
        ...
        if self.api_key:
            ...
            print("[WARNING] 未配置Tavily API Key")
            self.client = None

    def search_ted_talks(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        """
        搜索TED演讲
        ...
        Raises:
            ValueError: 如果API Key未配置或客户端未初始化
            Exception: 搜索失败且重试后仍失败
        """
        if not self.client:
            raise ValueError("Tavily客户端未初始化，请检查API Key配置")
        ...
        for attempt in range(max_retries):
            try:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    # 验证URL是否为有效的TED演讲
                    if self._is_valid_ted_talk_url(simplified_result['url']):
                        # 丰富TED信息（获取speaker、duration、views、description等）
                        enriched_result = self._enrich_ted_info(simplified_result)
    ...
    def _enrich_ted_info(self, basic_info: dict) -> dict:
        """
        ...
        try:
            ...
            if ted_data:
                ...
                # 如果获取详细信息完全失败，使用AI辅助演讲者提取
                return self._enrich_with_ai_assistance(basic_info)
            ...
            return {
                **basic_info,
                "speaker": self._extract_speaker_from_title(basic_info.get('title', '')),
                "duration": self._estimate_duration_from_content(basic_info.get('content', '')),
                "views": "未知",
                "description": basic_info.get('content', '暂无描述')[:200] + "..." if len(basic_info.get('content', '')) > 200 else basic_info.get('content', '暂无描述'),
                "has_transcript": False
            }

    def _enrich_with_ai_assistance(self, basic_info: dict) -> dict:
        """
        使用AI辅助丰富TED信息
        ...
        try:
            ...
            # 从URL提取演讲者候选
            speaker_from_url = self._extract_speaker_from_url(basic_info.get('url', ''))
            ...
            # 如果AI提取失败，使用备用方法
            if ai_speaker == "未知演讲者":
                traditional_speaker = self._extract_speaker_from_title(basic_info.get('title', ''))
    ...
    def _estimate_duration_from_content(self, content: str) -> str:
        """根据内容长度估计演讲时长"""
        try:
            if not content:
                return "15:00"  # 默认15分钟
    ...
    def _is_valid_ted_talk_url(self, url: str) -> bool:
        """
        验证URL是否为有效的TED演讲
...
def ted_tavily_search(query: str, max_results: int = 5) -> List[Dict[str, Any]]:
    ...
    注意：此函数使用全局配置，不推荐在新代码中使用。
    建议使用 TavilySearchService 类进行依赖注入。
    """
    from app.config import settings
    service = TavilySearchService(settings.tavily_api_key)
    return service.search_ted_talks(query, max_results)